...
export interface FuzzyMatchConfig {
  ...
  threshold: number;
  // Weight for each algorithm in the final score
  weights: {
    levenshtein: number;
    jaroWinkler: number;
    nGram: number;
  ...
  // Remove common words before comparison
 ...
 */
function normalizeText(text: string, config: FuzzyMatchConfig): string {
 ...
 */
function tokenBasedSimilarity(str1: string, str2: string): number {
  ...
  for (const token1 of tokens1) {
    ...
    for (const token2 of tokens2) {
      if (matchedTokens.has(token2)) continue;
    ...
    if (bestMatch > 0.5) { // Only count reasonable matches
      ...
      if (bestToken) matchedTokens.add(bestToken);
    }
    totalTokens++;
  }
 ...
 */
function createAggressiveKey(name: string): string {
...
export function findSimilarNames(
  ...
  nameList: string[],
  config: FuzzyMatchConfig = DEFAULT_CONFIG
): Array<{ name: string; result: FuzzyMatchResult }> {
