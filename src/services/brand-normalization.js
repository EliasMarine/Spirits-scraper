...
function removeStopWords(text) {
    const words = text.split(/\s+/);
    const filteredWords = words.filter(word => !BRAND_STOP_WORDS.has(word.toLowerCase()));
    return filteredWords.join(' ');
}
/**
 * Extract core brand name by removing common suffixes
 */
function extractCoreBrandName(text) {
 ...
 */
function findBestBrandMatch(normalizedName) {
 ...
 */
export function normalizeBrandName(brandName, config = DEFAULT_BRAND_CONFIG) {
 ...
 */
export function batchNormalizeBrands(brandNames, config = DEFAULT_BRAND_CONFIG) {
 ...
 */
export function getCanonicalBrandNames() {
    return Object.keys(KNOWN_BRANDS).sort();
}
/**
 * Add new brand mapping (for admin interface)
 */
export function addBrandMapping(canonical, variations) {
 ...
 */
export function findDuplicateBrands(brandNames, config = DEFAULT_BRAND_CONFIG) {
