import { z } from 'zod';
import { SpiritDataSchema } from '../types/index.js';
...
export class DescriptionTracker {
    static descriptions = new Map(); // fingerprint -> [spirit names]
    static addDescription(fingerprint, spiritName) {
...
export class DataValidator {
     ...
     */
    static createNormalizedKey(name) {
     ...
     */
    isDuplicate(spirit1, spirit2, threshold = 0.85) {
     ...
     */
    getDuplicateAnalysis(spirit1, spirit2) {
        const factors = [];
        // Name matching
        const nameMatch = fuzzyMatch(spirit1.name || '', spirit2.name || '', DEFAULT_CONFIG);
     ...
     */
    createDescriptionFingerprint(description) {
     ...
     */
    isGenericDescription(description, fingerprint) {
     ...
     */
    enhanceGenericDescription(description, name, brand) {
    ...
    validateVolume(volume) {
        ...
        const closest = standardSizes.reduce((prev, curr) => Math.abs(curr - value) < Math.abs(prev - value) ? curr : prev);
