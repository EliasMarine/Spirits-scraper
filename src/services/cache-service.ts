...
export class CacheService {
  ...
  private async setRedisCache<T>(key: string, data: CacheEntry<T>, ttlSeconds: number): Promise<void> {
    try {
  ...
  private async getRedisCache<T>(key: string): Promise<CacheEntry<T> | null> {
    try {
  ...
  private async loadCacheFile<T>(type: string): Promise<Map<string, CacheEntry<T>>> {
  ...
  private async saveCacheFile<T>(type: string, cache: Map<string, CacheEntry<T>>): Promise<void> {
    const filePath = this.getCacheFilePath(type);
    const data = JSON.stringify([...cache.entries()], null, 2);
    await fs.writeFile(filePath, data);
  }

  async cacheSearchQuery(query: string, options: any, results: any, customExpiry?: number): Promise<void> {
  ...
  async getCachedSearchQuery(query: string, options: any): Promise<any | null> {
  ...
  async cacheUrlContent(url: string, content: any, customExpiry?: number): Promise<void> {
  ...
  async getCachedUrlContent(url: string): Promise<any | null> {
    // If bypass mode is enabled, always return null to force fresh API calls
    if (this.bypassCache) {
      return null;
    }
  ...
  async hasUrlContent(url: string): Promise<boolean> {
    // Try Redis first
    if (this.useUpstash || this.useRedis) {
      const redisKey = this.generateRedisKey('url_content', url);
      const entry = await this.getRedisCache(redisKey);
  ...
  async get<T>(type: string, key: string): Promise<CacheEntry<T> | null> {
    ...
    if (this.useUpstash || this.useRedis) {
      ...
      const entry = await this.getRedisCache<T>(redisKey);
    ...
    // Fall back to file cache
    const cache = await this.loadCacheFile<T>(type);
  ...
  async set<T>(type: string, key: string, data: T, customExpiry?: number): Promise<void> {
  ...
  async cacheSpiritData(name: string, brand: string, data: any, customExpiry?: number): Promise<void> {
  ...
  async getCachedSpiritData(name: string, brand: string): Promise<any | null> {
    // If bypass mode is enabled, always return null to force fresh API calls
    if (this.bypassCache) {
      return null;
    }
  ...
  async markFailedAttempt(identifier: string, error: string, retryAfterHours: number = 4): Promise<void> {
  ...
  async isFailedAttempt(identifier: string): Promise<boolean> {
    const cache = await this.loadCacheFile('failed_attempt');
   ...
   */
  async get<T>(key: string, type: string = 'custom'): Promise<CacheEntry<T> | null> {
    // If bypass mode is enabled, always return null
    if (this.bypassCache) {
      return null;
    }
   ...
   */
  async set<T>(key: string, data: T, ttlMs: number = 24 * 60 * 60 * 1000, type: string = 'custom'): Promise<void> {
  ...
  async getStats(): Promise<CacheStats & { redisEnabled: boolean; cacheType: string; metrics: CacheMetrics }> {
    const types = ['search_query', 'url_content', 'spirit_data', 'failed_attempt'];
    let totalEntries = 0;
    let sizeBytes = 0;
