...
export class GoogleSearchClient {
   ...
   */
  async search(options: SearchQueryOptions): Promise<GoogleSearchResponse> {
    ...
    try {
      ...
      const response = await this.client.get<GoogleSearchResponse>('', { params });
   ...
   */
  async searchImages(spiritName: string, brand?: string): Promise<string[]> {
