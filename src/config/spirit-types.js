 ...
 */
export const SPIRIT_TYPE_CONFIG = {
    version: '1.0.0',
    types: {
 ...
 */
export function detectSpiritType(name, brand, description) {
    ...
    for (const [typeName, config] of sortedTypes) {
        ...
        const hasBrandIndicator = config.brandIndicators?.some(indicator => searchText.includes(indicator.toLowerCase()));
 ...
 */
export function getWhiskeyStyle(name, description) {
 ...
 */
export function validateSpiritType(detectedType, name, brand) {
    ...
    const matchesBrand = config.brandIndicators?.some(indicator => searchText.includes(indicator.toLowerCase()));
