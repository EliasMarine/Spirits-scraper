...
export interface QueryTemplate {
  type: 'basic' | 'limited' | 'retailer' | 'historical' | 'discovery';
  template: string;
  priority: number;
}
...
// Type alias for consistency with other files
export type DistilleryConfig = Distillery;

// Query template system for comprehensive scraping
export const QUERY_TEMPLATES = {
  basic: (brand: string, product_type: string = '') => `"${brand}" ${product_type}`.trim(),
  limited: (brand: string) => `"${brand}" limited edition special release exclusive`,
  retailer: (brand: string, domain: string) => `site:${domain} "${brand}"`,
  historical: (brand: string) => `"${brand}" discontinued vintage rare collectible`,
  discovery: (brand: string, exclude: string[]) => `"${brand}" -${exclude.join(' -')}`,
  product_line: (brand: string, line: string) => `"${brand}" "${line}"`,
  year_range: (brand: string, start: number, end: number) => {
    const years = [];
  ...
  cask_finish: (brand: string) => `"${brand}" cask finish barrel proof sherry port wine`,
  collaboration: (brand: string) => `"${brand}" collaboration partnership exclusive release`
};
...
export const DISCOVERY_STRATEGIES = {
  ...
  temporal: {
    ...
    generateQueries: (distillery: Distillery) => {
  ...
  retailer_mining: {
    ...
    generateQueries: (distillery: Distillery, retailer: string) => 
      `site:${retailer} "${distillery.name}" -cart -checkout`
  },
  ...
  auction_discovery: {
    ...
    generateQueries: (distillery: Distillery, site: string) => 
      `site:${site} "${distillery.name}" lot auction`
  },
...
export function getAllDistilleryNames(): string[] {
  const names: string[] = [];
...
export function getRandomDistilleries(count: number): Distillery[] {
  const shuffled = [...DISTILLERIES].sort(() => 0.5 - Math.random());
  return shuffled.slice(0, count);
}

// Helper function to find a distillery by name (including variations)
export function findDistillery(name: string): Distillery | undefined {
  const normalizedName = name.toLowerCase().trim();
  return DISTILLERIES.find(d => 
...
export const ALL_DISTILLERIES: Distillery[] = [
  ...DISTILLERIES,
