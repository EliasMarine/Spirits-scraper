...
export const SEARCH_TEMPLATES = {
  spiritInfo: (name: string, brand?: string) =>
    `${name} ${brand || ''} bottle whiskey bourbon spirit alcohol ABV proof ${SEARCH_EXCLUSIONS}`.trim(),

  spiritReview: (name: string, brand?: string) =>
    `${name} ${brand || ''} review tasting notes flavor profile whiskey bourbon ${SEARCH_EXCLUSIONS}`.trim(),

  spiritRetailer: (name: string, brand?: string) =>
    `${name} ${brand || ''} buy bottle 750ml online price ${SEARCH_EXCLUSIONS}`.trim(),

  categorySearch: (category: string) =>
    `${category} bottle buy online price 750ml ${SEARCH_EXCLUSIONS}`,

  brandCatalog: (brand: string) =>
    `${brand} whiskey bourbon bottle buy online ${SEARCH_EXCLUSIONS}`,
};