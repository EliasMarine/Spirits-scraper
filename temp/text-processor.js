...
exports.isIncompleteExtraction = exports.containsStoreReference = exports.processSpirit = exports.normalizeBrandName = exports.isValidProductDescription = exports.extractValidAge = exports.normalizeCategory = exports.removeStoreSuffixes = exports.removeNavigationPrefixes = exports.removeStoreNames = exports.fixTextSpacing = exports.TextProcessor = void 0;
var TextProcessor = /** @class */ (function () {
     ...
     */
    TextProcessor.removeNavigationPrefixes = function (text) {
        if (!text)
            return '';
        var result = text;
        ...
        for (var _i = 0, prefixPatterns_1 = prefixPatterns; _i < prefixPatterns_1.length; _i++) {
            var pattern = prefixPatterns_1[_i];
     ...
     */
    TextProcessor.removeStoreSuffixes = function (text) {
        if (!text)
            return '';
        var result = text;
        ...
        for (var _i = 0, suffixPatterns_1 = suffixPatterns; _i < suffixPatterns_1.length; _i++) {
            var pattern = suffixPatterns_1[_i];
            result = result.replace(pattern, '');
        }
        return result.trim();
    };
     ...
     */
    TextProcessor.fixTextSpacing = function (text) {
        if (!text)
            return '';
        var result = text;
        ...
        var hasProperSpacing = /\s/.test(text.trim());
        ...
        result = result.replace(/(\d+)\s+(st|nd|rd|th)\b/gi, function (match, num, suffix) {
        ...
        result = result.replace(/\b([A-Z])\s+([a-z]{1,3})\b/g, function (match, letter, suffix) {
            ...
            var key = "".concat(letter, " ").concat(suffix);
     ...
     */
    TextProcessor.removeStoreNames = function (text) {
        if (!text)
            return '';
        var result = text;
        ...
        // Apply all patterns
        for (var _i = 0, storePatterns_1 = storePatterns; _i < storePatterns_1.length; _i++) {
            var pattern = storePatterns_1[_i];
            result = result.replace(pattern, '');
        }
     ...
     */
    TextProcessor.normalizeCategory = function (name, currentCategory) {
        if (!name)
            return currentCategory || 'Other'; // Return 'Other' not 'other'
        var lowerName = name.toLowerCase();
        console.log("\uD83D\uDD0D TextProcessor.normalizeCategory: name=\"".concat(name, "\", currentCategory=\"").concat(currentCategory, "\""));
        ...
        for (var _i = 0, bourbonBrands_1 = bourbonBrands; _i < bourbonBrands_1.length; _i++) {
            var brand = bourbonBrands_1[_i];
            if (lowerName.includes(brand)) {
                console.log("\u2705 TextProcessor: Found bourbon brand \"".concat(brand, "\" -> Bourbon"));
        ...
        for (var _a = 0, _b = Object.entries(this.CATEGORY_PATTERNS); _a < _b.length; _a++) {
            var _c = _b[_a], category = _c[0], patterns = _c[1];
        ...
        if (currentCategory) {
            var cleanCategory = currentCategory.toLowerCase().trim();
     ...
     */
    TextProcessor.extractValidAge = function (text) {
        if (!text)
            return null;
        ...
        for (var _i = 0, noAgeStatementBrands_1 = noAgeStatementBrands; _i < noAgeStatementBrands_1.length; _i++) {
            var pattern = noAgeStatementBrands_1[_i];
        ...
        var _loop_1 = function (pattern) {
            ...
            if (match) {
                ...
                var contextBefore_1 = text.substring(Math.max(0, match.index - 50), match.index);
                var contextAfter = text.substring(match.index + match[0].length, Math.min(text.length, match.index + match[0].length + 50));
                // Skip if the context suggests it's not an age
                var skipContextPatterns = [
                ...
                if (skipContextPatterns.some(function (p) { return p.test(contextBefore_1); })) {
                    return "continue";
                }
                // Validate age is reasonable for spirits
                if (age >= 2 && age <= 30) {
                    return { value: "".concat(age, " Year") };
                }
        ...
        for (var _a = 0, agePatterns_1 = agePatterns; _a < agePatterns_1.length; _a++) {
            var pattern = agePatterns_1[_a];
        ...
        var vintageMatch = text.match(/\b(19\d{2}|20[0-2]\d)\s*vintage/i);
        if (vintageMatch) {
            var year = parseInt(vintageMatch[1], 10);
            var currentYear = new Date().getFullYear();
            var age = currentYear - year;
            if (age >= 1 && age <= 100) {
                return "".concat(year, " Vintage");
            }
        }
        return null;
    };
     ...
     */
    TextProcessor.isValidProductDescription = function (desc) {
        if (!desc || desc.length < 20)
            return false;
        var lowerDesc = desc.toLowerCase();
        // Check for review indicators
        for (var _i = 0, _a = this.REVIEW_INDICATORS; _i < _a.length; _i++) {
            var pattern = _a[_i];
        ...
        if (/^\$?\d+\.?\d*$/.test(desc.trim()))
            return false;
        if (/^(in stock|out of stock|available|unavailable)/i.test(desc.trim()))
            return false;
        if (/^(buy now|shop now|order now)/i.test(desc.trim()))
            return false;
        // Count product description indicators
        var productIndicatorCount = 0;
        for (var _b = 0, _c = this.PRODUCT_INDICATORS; _b < _c.length; _b++) {
            var pattern = _c[_b];
     ...
     */
    TextProcessor.normalizeBrandName = function (brand) {
        if (!brand)
            return '';
        var result = brand.trim();
        // Fix spacing first
        ...
        result = result.split(/\s+/).map(function (word, index) {
            // Preserve certain words in lowercase (unless first word)
            var lowerWords = ['de', 'del', 'la', 'el', 'y', 'and', '&', 'of', 'the'];
     ...
     */
    TextProcessor.processSpirit = function (spirit) {
        // Fix text spacing in name
        var fixedName = this.fixTextSpacing(spirit.name);
        ...
        // Normalize category
        var normalizedCategory = this.normalizeCategory(fixedName, spirit.category);
        // Extract age if not provided
        var age = spirit.age || this.extractValidAge(fixedName);
        // Validate and clean description
        var validDescription = null;
        if (spirit.description) {
            var fixedDescription = this.fixTextSpacing(spirit.description);
     ...
     */
    TextProcessor.extractBrandFromName = function (name) {
        ...
        if (/^cognac\s+([A-Z][a-zA-Z\s&'.-]+?)(?:\s+XO|\s+VSOP|\s+VS|\s+Napoleon|\s+Extra|\s+Paradis|\s+Richard)/i.test(name)) {
            var match = name.match(/^cognac\s+([A-Z][a-zA-Z\s&'.-]+?)(?:\s+XO|\s+VSOP|\s+VS|\s+Napoleon|\s+Extra|\s+Paradis|\s+Richard)/i);
            if (match) {
        ...
        if (/^([A-Z][a-zA-Z\s&'.-]+?)(?:\s+XO|\s+VSOP|\s+VS|\s+Napoleon|\s+Extra)\s+(?:Cognac|Brandy|Armagnac)/i.test(name)) {
            var match = name.match(/^([A-Z][a-zA-Z\s&'.-]+?)(?:\s+XO|\s+VSOP|\s+VS|\s+Napoleon|\s+Extra)\s+(?:Cognac|Brandy|Armagnac)/i);
            if (match) {
                return this.normalizeBrandName(match[1]);
            }
        }
        ...
        for (var _i = 0, brandPatterns_1 = brandPatterns; _i < brandPatterns_1.length; _i++) {
            var pattern = brandPatterns_1[_i];
        ...
        if (words.length >= 2) {
            var potentialBrand = words.slice(0, Math.min(3, words.length)).join(' ');
     ...
     */
    TextProcessor.containsStoreReference = function (text) {
        if (!text)
            return false;
        var storePatterns = [
        ...
        return storePatterns.some(function (pattern) { return pattern.test(text); });
    };
    /**
     * V2.7.4: Check if name extraction seems incomplete
     */
    TextProcessor.isIncompleteExtraction = function (name, description) {
        if (!name || !description)
            return false;
        ...
        if (name.length < 10 && description.length > 100) {
            ...
            for (var _i = 0, productPatterns_1 = productPatterns; _i < productPatterns_1.length; _i++) {
                var pattern = productPatterns_1[_i];
        ...
        return incompletePatterns.some(function (pattern) { return pattern.test(name.trim()); });
    };
    // Common spirit categories with variations
    TextProcessor.CATEGORY_PATTERNS = {
        whiskey: [
            /whiskey|whisky/i,
            /bourbon/i,
            /scotch/i,
    ...
    TextProcessor.REVIEW_INDICATORS = [
    ...
    TextProcessor.PRODUCT_INDICATORS = [
        /aged\s+(\d+|in|for)/i,
...
exports.fixTextSpacing = TextProcessor.fixTextSpacing, exports.removeStoreNames = TextProcessor.removeStoreNames, exports.removeNavigationPrefixes = TextProcessor.removeNavigationPrefixes, exports.removeStoreSuffixes = TextProcessor.removeStoreSuffixes, exports.normalizeCategory = TextProcessor.normalizeCategory, exports.extractValidAge = TextProcessor.extractValidAge, exports.isValidProductDescription = TextProcessor.isValidProductDescription, exports.normalizeBrandName = TextProcessor.normalizeBrandName, exports.processSpirit = TextProcessor.processSpirit, exports.containsStoreReference = TextProcessor.containsStoreReference, exports.isIncompleteExtraction = TextProcessor.isIncompleteExtraction;
