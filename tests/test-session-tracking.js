#!/usr/bin/env ts-node
/**
 * Comprehensive test script for V2.5.2 Smart Session Tracking
 *
 ...
 * 4. Different category (should not skip)
 * 5. After clearing session (should not skip)
...
import { apiCallTracker } from './src/services/api-call-tracker.js';
// Test configuration
...
async function runTest(testName, category, limit, expectSkip = false, clearSession = false) {
    ...
    try {
        ...
        const apiUsageBefore = apiCallTracker.getDailyUsage();
        ...
        const apiUsageAfter = apiCallTracker.getDailyUsage();
...
async function runAllTests() {
    ...
    try {
        ...
        await cacheService.clearNamespace('scrape_session');
        ...
        await runTest('Initial Scrape (Baseline)', TEST_CATEGORY, SMALL_LIMIT, false, // Should not skip
        false // Don't clear session
        );
        // Wait a bit to ensure cache is written
        await new Promise(resolve => setTimeout(resolve, 2000));
        // 2. Same category, same limit - should skip
        await runTest('Same Category & Limit (Should Skip)', TEST_CATEGORY, SMALL_LIMIT, true, // Should skip
        false // Don't clear session
        );
        // 3. Same category, higher limit - should fetch more
        await runTest('Same Category, Higher Limit (Should Fetch More)', TEST_CATEGORY, LARGE_LIMIT, false, // Should not skip (needs more spirits)
        false // Don't clear session
        );
        // 4. Same category, same higher limit again - should skip
        await runTest('Same Category & Higher Limit Again (Should Skip)', TEST_CATEGORY, LARGE_LIMIT, true, // Should skip now
        false // Don't clear session
        );
        // 5. Different category - should not skip
        await runTest('Different Category (Should Not Skip)', ALT_CATEGORY, SMALL_LIMIT, false, // Should not skip
        false // Don't clear session
        );
        // 6. Back to first category after clearing session - should not skip
        await runTest('After Clearing Session (Should Not Skip)', TEST_CATEGORY, SMALL_LIMIT, false, // Should not skip
        ...
        await runTest('Create Session for Persistence Test', 'vodka', SMALL_LIMIT, false, true // Clear any existing session
        );
        ...
        await runTest('After Simulated Restart (Should Skip)', 'vodka', SMALL_LIMIT, true, // Should skip
        false // Don't clear session
        );
    }
    catch (error) {
