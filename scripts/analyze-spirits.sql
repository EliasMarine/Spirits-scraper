-- ============================================
-- Spirit Database Analysis Queries
-- ============================================
-- Use these queries to analyze spirits data and improve algorithms
...
-- 3. Category and Type Distribution
-- ---------------------------------
SELECT 
...
-- 4. Sample Spirits for Algorithm Testing (Various Patterns)
-- ---------------------------------------------------------
SELECT 
    id,
    brand,
...
-- 5. Potential Duplicate Patterns (Same Brand, Similar Names)
-- -----------------------------------------------------------
WITH name_patterns AS (
    SELECT 
        ...
        CASE 
            WHEN name ~* '(\d{4})\s*release' THEN 
                regexp_replace(name, '(\d{4})\s*release', 'YYYY release', 'gi')
            ELSE name 
        END as release_normalized
    FROM spirits
    WHERE brand IS NOT NULL
)
SELECT 
...
-- 6. Spirits with Missing Critical Data (for data quality improvement)
-- -------------------------------------------------------------------
SELECT 
    id,
    brand,
    ...
    ARRAY_LENGTH(ARRAY_REMOVE(ARRAY[
        CASE WHEN brand IS NULL THEN 'brand' END,
    ...
    ], NULL), 1) as missing_fields_count,
    ARRAY_REMOVE(ARRAY[
        CASE WHEN brand IS NULL THEN 'brand' END,
        CASE WHEN type IS NULL THEN 'type' END,
        CASE WHEN category IS NULL THEN 'category' END,
...
-- 7. Brand Name Variations (potential normalization targets)
-- ---------------------------------------------------------
...
ORDER BY age::INTEGER DESC NULLS LAST;

-- 9. Recent Additions (for monitoring scraping quality)
-- ---------------------------------------------------
SELECT 
...
-- 10. Spirits Name Patterns for Discovery Algorithm
-- ------------------------------------------------
SELECT 
    CASE 
